    ...
    for feature in "${features[@]}"; do
      for idx in "${!pred_lens[@]}"; do  # Loop through indices of pred_lens
        ...
        if [ "$feature" = "S" ]; then
          ...
          python -u run.py --model $model --data $target --data_path close_data.csv --features $feature --dec_in 1 --enc_in 1 --c_out 1 --target $target --pred_len $pred_len --seq_len $seq_len --label_len $label_len --freq h --use_gpu False
        else
          # If features are MS, set enc_in, dec_in, and c_out to 8
          python -u run.py --model $model --data $target --data_path close_data.csv --features $feature --dec_in 8 --enc_in 8 --c_out 8 --target $target --pred_len $pred_len --seq_len $seq_len --label_len $label_len --freq h --use_gpu False
        fi
      done
    done
  done
